...
class create_regulator_button:
      ...
      def create_buttons(self):
       ...
       stop= Button(root,text = "STOP",padx = 20, pady = 20,command=self.stop)
       stop.config(state=DISABLED)
       stop.place(x = 400,y = 50)
     ...
     #This method fast forwards the song
      ...
      def display_eng(self):
          album = Button(root,text = "ALBUM",padx = 20, pady = 20,command=self.db_manager.album_display_eng)
      ...
      def display_Hindi(self):
          album = Button(root,text = "ALBUM",padx = 20, pady = 20,command=self.db_manager.album_display_hindi)
...
class databaseManagement:
    def __init__(self):
     ...
     #Displays the different artists
    ...
    #Displays the songs belonging to an artist
    ...
    #Display the albums for english songs
    def album_display_eng(self):
        border_frame = Frame(root, relief=SOLID, borderwidth=2)
        border_frame.place(x=800, y=150)  # Adjust the x and y coordinates as needed
        # Create a label widget inside the border frame

        artist_label = Label(border_frame, text="ALBUM")
        artist_label.pack(padx=30, pady=10)
        mycursor.execute('select albumName from album where albumName not like "%HINDI"')
        result = mycursor.fetchall()
        ...
        for x in range(len(result)):
            ...
            #This lambda function would only execute when the button of a specific artist is clicked
            artist_name = Button(root, text=res, padx=70, pady=20,command=lambda r=res: self.album_song_list_eng(r))
            artist_name.place(x=790,y=210+(x*70))
    
    #Displays the songs of hindi albums
    def album_song_list_eng(self,result):
        alist=[]
        mycursor.execute("SELECT songName FROM songs WHERE albumID = (SELECT albumID FROM album WHERE albumName = %s)", (result,))
        ans = mycursor.fetchall()
        ...
        for x in range(len(alist)):
            song_name = Button(root,text=alist[x],padx=20,pady=20,command=lambda r = alist[x]:self.QuerySelection(r))
            song_name.place(x=200,y=490+(x*70))


    #Display the albums for hindi songs
    def album_display_hindi(self):
        border_frame = Frame(root, relief=SOLID, borderwidth=2)
        border_frame.place(x=800, y=150)  # Adjust the x and y coordinates as needed
        # Create a label widget inside the border frame
        ...
        artist_label = Label(border_frame, text="ALBUM")
        artist_label.pack(padx=30, pady=10)
        mycursor.execute('select albumName from album where albumName like "%HINDI"')
        result = mycursor.fetchall()

        for x in range(len(result)):
            res = result[x][0]
            
            #Removes the "_HINDI" part from the album name
            res_accurate = res.replace("_HINDI","")
            
            #This lambda function would only execute when the button of a specific artist is clicked
            artist_name = Button(root, text=res_accurate, padx=70, pady=20,command=lambda r=res: self.album_song_list_hindi(r))
            artist_name.place(x=790,y=210+(x*70))
    
    #Displays the songs of english albums
    def album_song_list_hindi(self,result):
        alist=[]
        mycursor.execute("SELECT songName FROM songs WHERE albumID = (SELECT albumID FROM album WHERE albumName = %s)", (result,))
        ans = mycursor.fetchall()
    ...
    def QuerySelection(self,song_name):
        ...
        # Enable the buttons after the song starts playing
        self.enable_buttons()
